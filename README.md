...
合盖是硬件事件触发的强制休眠，IOKit 电源断言拦不住它。系统里唯一的开关是
`pmset -a disablesleep`，需要 root，所以偏好里的「盒盖不休眠」在开关时会弹一次
系统自带的管理员授权框（密码不经过 MacAwake，由系统的授权服务处理）。

这是个**系统级**开关，独立于上面所有模式：

- 打开后不管有没有别的保持唤醒理由，合盖都不休眠
- 退出 MacAwake、甚至重启之后依然生效，必须手动关掉
...
- **合盖休眠要单独开。** 电源断言只挡闲置休眠；挡合盖得用「盒盖不休眠」，它改的是系统设置并需要管理员授权，开着就一直生效。
- 电量过低时系统会强制休眠，断言无效。
- 服务检测只覆盖 TCP，不含 UDP。
